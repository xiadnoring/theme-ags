...
if [[ "$1" = "#"* ]]; then # this is a color
    ...
    > "$CONFIG_DIR"/scss/_material.scss
    if [ "$2" = "--apply" ]; then
        #cp "$CONFIG_DIR"/scss/_material.scss "$CONFIG_DIR/scss/_material.scss"
    ...
    # copy scss
    cp "$XDG_CACHE_HOME/wal/colors.scss" "$CONFIG_DIR"/scss/_material.scss

    cat color_generation/pywal_to_material.scss >> "$CONFIG_DIR"/scss/_material.scss
    
    # if [ "$2" = "--apply" ]; then
    #     sass -I "$CONFIG_DIR/style.scss" -I --style compressed "$CONFIG_DIR/scss/generated/main.css"
